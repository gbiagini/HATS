...
my $gene = DRB5_INFO::DRB5();
my $ciwd_ref = ORGANIZE::CIWD( $gene );
my $cwd_ref = ORGANIZE::CWD( $gene );
my $ecwd_ref = ORGANIZE::EURCWD( $gene );

my $leader = DRB5_INFO::DRB5_LEADER();
my $ref_ref = DRB5_INFO::REF("ALL");
my $residues_ref = DRB5_INFO::RESIDUES("ALL");
my $partial_ref = DRB5_INFO::PARTIAL();

my $group_ref = DRB5_INFO::GROUP();
my $base_ref = DRB5_INFO::BASE();
my $basetype_ref = DRB5_INFO::BASETYPE();
...
for ( my $index = 0; $index < scalar @group; $index++ ) {
	$ref_ref = DRB5_INFO::REF( $group[ $index ] );
	$residues_ref = DRB5_INFO::RESIDUES( $group[ $index ] );
...
#$ref_ref = DRB5_INFO::REF("DR51");
#$residues_ref = DRB5_INFO::RESIDUES("DR51");
#my $dr51_ref = ASSIGN::ASSIGN($fasta_ref, $all_ref, $gene, $leader,$ref_ref, $residues_ref);

my $unassigned_ref = ASSIGN::UNASSIGNED( $fasta_ref, $assigned_ref, $gene );

@csv = glob('output/*.csv');
my $sero_ref = DRB5_INFO::SERO();
my $key_ref = DRB5_INFO::KEY();
